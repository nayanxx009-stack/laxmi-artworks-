...
// First remove the existing block
const signoutBlockRegex = /\{showSignOutConfirm && \([\s\S]*?\}\)\}/;
code = code.replace(signoutBlockRegex, '');

// Now add it at the very end of the return statement
code = code.replace(
  /<\/div>\n  \);\n\}/,
  `
      {showSignOutConfirm && (
